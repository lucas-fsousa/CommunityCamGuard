...
START arriving before ACCEPT across the independent channels still fails closed;
bounded cross-channel reorder support remains pending. The historical paired replay
and earlier codec validations remain separate evidence, not proof of this owner's
live handshake or physical camera interoperability. Next: implement bounded reorder
handling and a short camera-3-only socket adapter experiment before any source switch.
