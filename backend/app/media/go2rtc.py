...
def web_stream_id(camera_id: str) -> str:
    """On-demand downscaled live variant, derived from the shared base restream.
...
def hd_stream_id(camera_id: str) -> str:
    ...
    core per camera. It runs only while consumed and is shared by all browser consumers;
    it never opens an additional camera session.
    """
...
def build_config(cameras: list[Camera] | None = None) -> dict:
    ...
    for cam in cameras:
        ...
        # Independent on-demand variants of the same base restream: SD must not depend on HD,
        # otherwise a reduced-quality viewer keeps TWO encoders alive. No preload pins either
        # variant. go2rtc stops each producer when its last consumer detaches; different clients
        # can still legitimately consume different qualities without interrupting one another.
        # SD reads the original camera timeline too, so it needs the same #async clock repair.
        sd_vid = quality.video_h264_directive(s.live_hwaccel, codec="h264_sd")
        streams[web_stream_id(cam.camera_id)] = (
            f"ffmpeg:{sid}#async{sd_vid}{audio_part}{sub_raw}")
    return {
...
class Go2rtc:
    ...
    def ensure_external_config(self, cameras: list[Camera] | None = None) -> bool:
        """Apply changed config, avoiding a destructive no-op restart at app startup.

        go2rtc's in-process restart can leave an old FFmpeg producer alive while reconnecting
        dashboard consumers create its replacement. Rebuilding only the app container must not
        therefore restart a healthy external media engine when the generated bytes are unchanged.
        """

        if self.write_config_if_changed(cameras):
            return self.reload_external()
        return self.wait_healthy(timeout=5)
    ...
    def release_live_producers(self, camera_id: str, *, disconnect_grace: float = 2.0) -> bool:
        """Allow detached live producers to stop, without pinning a replacement.

        The player disconnects first. Remove any legacy preload for BOTH local qualities, then
        allow the relay to detach. The next actual viewer starts the requested variant. Existing
        viewers retain their shared producer; recovery must not disconnect other clients or the
        base recording source. Never PUT a preload here, even on the explicit recovery path.
        """
        variants = (hd_stream_id(camera_id), web_stream_id(camera_id))
        try:
            with urllib.request.urlopen(f"{self.api}/api/preload", timeout=3) as response:
                preloads = json.loads(response.read()) or {}
            if not isinstance(preloads, dict):
                return False
        except (urllib.error.URLError, OSError, ValueError):
            return False
        for sid in variants:
            if sid not in preloads:
                continue
            endpoint = f"{self.api}/api/preload?{urlencode({'src': sid})}"
            try:
