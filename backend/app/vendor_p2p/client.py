"""Bounded, read-only IoTVideo access-node and rendezvous client.

The inventory probe stops after certification, account-device inventory, heartbeat and TermDNS.
The separate route probe performs the brokered CALLING and direct CA/CB NAT handshake, but does not
write a thing-model property, open media, send application data or use the device subscription
token.  Secrets and peer routes never leave this module.
