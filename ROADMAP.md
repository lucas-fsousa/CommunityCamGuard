...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Paired receive replay reached active with 151 video records/155 audio frames in ten seconds. Bounded reliable INIT/START sender and paired receiver are now composed in a socket-free handshake owner: INIT receipt, ACCEPT/peer START, local START receipt and encoding readiness stay separate; early records/parser state survive negotiation. Nine integrated tests added. Raw timestamps are not UTC; no packet transmission or production switch. Next: bounded cross-channel reorder, camera-3 socket/reception experiment, reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
