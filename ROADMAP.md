...
| P1 | Recover ambiguous floodlight writes using bounded, strictly correlated state reads when the write reply is missing; never replay the actuation or suppress explicit rejection. Camera-3 logs identified this failure stage; live recovery remains to be observed in a user-triggered operation | wip |
