...
| P1 | Camera-card recordings — generic opaque-ID/UTC/bounded-page contracts and fail-closed Yoosee `tfInfo` parsing are implemented. Camera 3 now has a real card: repeated read-only probes return APK-defined normal status `1`, a stable card ID and coherent capacity/free values; the parser handles the real `stVal`/integral-double envelope and retains firmware 40.1.14's signed-total quirk for diagnostics. The feature stays hidden until a listing probe proves this exact camera/profile. Modern IoTVideo commands `0/16/18/19..27` are mapped; the exact 43-byte V2 command-16 request and its header/type-table/9-byte-item response have bounded tested codecs. The active legacy 16-byte listing request is also recovered. Live type vocabulary, command carrier and download framing remain; V3/V4, delete (`28/29`) and format stay disabled | wip |
