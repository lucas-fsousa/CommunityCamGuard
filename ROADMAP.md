...
| P1 | Camera-card recordings — add a driver-neutral onboard-storage source and a Yoosee adapter for `tfInfo`, UTC date/type listing, paginated files and bounded download/playback. Advertise it only after positive capacity/status plus a verified model/profile or successful read-only probe; all current units correctly remain hidden because they report no card. Modern IoTVideo commands `0/16/18/19..27` and legacy `iGetRecFiles` are statically mapped. Live homologation requires an authorized camera with a card; delete (`28/29`) and format remain out of scope | wip |
