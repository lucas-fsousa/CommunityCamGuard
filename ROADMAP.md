...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR restart gate passed three 120-frame fresh-decoder recovery scenarios; ungated negative control failed (74/120). Socket-free receive lifecycle and single-channel ACCEPT/TLV/V1 composition are synthetically tested, without production callers. Historical replay distinguishes ACCEPT and START/media on separate inbound KCP conversations: do not merge their sequence spaces or treat START as ACCEPT. Raw timestamps are not UTC. No live production switch. Next: two-channel negotiation coordinator and outbound INIT/ACK ownership, camera-3 reception, reconnect/pacing/A-V sync and bounded single-producer handoff. See `docs/internal/native-av-receive.md`, `native-receive-lifecycle.md`, `native-video-recovery.md` and `native-media-receive.md` | wip |
