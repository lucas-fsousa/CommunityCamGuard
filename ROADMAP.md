...
| P1 | Integrate proven P2P controls into reusable backend/Docker drivers — reflector, orientation and bounded siren pulse are isolated feature modules with typed LAN-only API/UI surfaces addressed by opaque `camera_id`. Explicit stale-session renewal is bounded to one retry and uses only the encrypted native account; concurrent operations are rejected per camera and Yoosee routes receive a measured 1.5-second settle window between sequential sessions. Camera 3 reflector read and reversible orientation cycle are production-validated; bounded siren validation remains | wip |
