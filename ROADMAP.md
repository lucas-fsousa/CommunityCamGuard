...
| P2 | PTZ credential/session longevity: determine the vendor-provided `expireTime` semantics separately from the local route-cache limits (8s idle / 20s absolute). Credentials are already encrypted and persisted; native PTZ currently does not use the one-shot stale-access renewal helper. Add bounded renewal only on explicit expired-credential rejection during preflight, rebuild the cache key from refreshed credentials, and never replay movement after START. Homologate longer route reuse with camera-3 resource/STOP tests before raising limits; no indefinite sockets or automatic movement retries | todo |
| P2 | Discreet dismissible notifications for camera-control outcomes: right-side responsive toasts, 5-second timeout, explicit localized close button, three-message cap and active-dialog scope implemented. Keep applying spinners and form-validation guidance inline. DOM contracts passed; real browser/mobile visual review and deployment pending | wip |
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR restart gate passed three 120-frame fresh-decoder recovery scenarios; ungated negative control failed (74/120). Explicit paired-channel receive coordinator now correlates ACCEPT on the high-bit conversation and START/media on the base conversation without merging sequence spaces. Ten-second historical replay reached active with 151 video records, 155 audio frames and zero pending bytes; ACKs not sent, no live handshake claim. Raw timestamps are not UTC. No production switch. Next: outbound INIT/START/ACK ownership, bounded cross-channel reorder, camera-3 reception, reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-paired.md`, `native-av-receive.md`, `native-video-recovery.md` and `native-media-receive.md` | wip |
...
| P2 | Audit unfinished vendor-driver scaffolds (including `hikvision.py`, `dahua.py` and `xiongmai.py`): inventory implemented versus placeholder behavior, document support/remaining work, and organize vendor-specific code into corresponding driver packages with clear responsibilities. Preserve registry/import compatibility, generic RTSP/ONVIF fallback and simultaneous multi-brand operation; add regression tests and ensure scaffolds never grant unimplemented capabilities. Keep shared contracts/core brand-agnostic. Requested 2026-09-15; audit and migration not yet performed | todo |
