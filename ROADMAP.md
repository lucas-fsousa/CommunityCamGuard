...
| P1 | Recover ambiguous floodlight writes using bounded, strictly correlated state reads when the write reply is missing; never replay the actuation or suppress explicit rejection. September 13 live camera-3 audit still failed ON with 502 and subsequent OFF readback: exact-unit actuation defect remains, not merely UI feedback. See `docs/internal/camera3-controls-audit-2026-09-13.md` | wip |
| P1 | Camera-3 live control audit: eight semantic controls, two intercom endpoints and four PTZ directions exercised sequentially, original settings restored. Light failed; other API/state checks passed, but physical output and browser interactions remain unverified. Obtain visual/acoustic confirmation before marking full homologation | wip |
| P1 | Diagnose intermittent white-light 502 failures using new phase logs on the next operator attempt; do not replay camera writes automatically or infer rejection from a missing reply | todo |
