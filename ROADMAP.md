...
| P1 | Camera-card recordings — generic opaque-ID/UTC/bounded-page contracts and fail-closed Yoosee `tfInfo` parsing are implemented. Camera 3's real card repeatedly returns APK-defined normal status `1`, a stable card ID and coherent capacity/free values; falling free space proves active onboard recording. The parser handles the real `stVal`/integral-double envelope and retains firmware 40.1.14's signed-total quirk. The feature stays hidden until a listing probe proves this exact camera/profile. Exact bounded codecs and a shared B9 read transport now cover modern V1 command `0`, V2-V4 command `16`, V2-V4 date command `18`, and internal V3/V4 recording-type command `15`, including complete V4 fragment assembly. The native selector is recovered: file/date minimum V2, type-window minimum V3, descending windows from 49.71 days promote to V3, and an authoritative device-platform enum `2` promotes to V4. The earlier forced 90-day V2 date probe was therefore not app-equivalent; exactly one bounded V3 date probe is justified, while blind version fan-out and command 15 remain disabled. The base APK's declared IoT playback implementation (`IotPlaybackCompoImpl`) is absent from all seven DEX files and its cross-platform playback entry is a throwing stub; only the generic `LocalPlaybackFragment -> GLocalPlaybackVmImpl -> GPlaybackKits -> GwMsgGetSDAvList` implementation is bundled. That legacy manager parses IDs as signed Java `int` while camera 3 is classified as IoT and has a greater-than-32-bit ID. Recover the missing solution component or obtain a physical-app SD playback trace before selecting a transport. Delete (`28/29`) and format stay disabled | wip |
