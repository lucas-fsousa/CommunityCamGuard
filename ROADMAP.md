...
| P1 | Camera-card recordings — generic opaque-ID/UTC/bounded-page contracts and fail-closed Yoosee `tfInfo` parsing are implemented. Camera 3's real card repeatedly returns APK-defined normal status `1`, a stable card ID and coherent capacity/free values; falling free space proves active onboard recording. The parser handles the real `stVal`/integral-double envelope and retains firmware 40.1.14's signed-total quirk. The feature stays hidden until a listing probe proves this exact camera/profile. Exact bounded codecs and a shared B9 read transport now cover modern V1 command `0`, V2-V4 command `16`, V2-V4 date command `18`, and internal V3/V4 recording-type command `15`, including complete V4 fragment assembly. The native selector is recovered: file/date minimum V2, type-window minimum V3, descending windows from 49.71 days promote to V3, and device-platform enum `2` promotes to V4. Camera 3's captured MTP flags prove platform 1; its selector-faithful 90-day V3 date request was transport/application ACKed without a page, so modern version probing is closed and command 15 remains disabled. The legacy command `3→4` binary response and exact camera-local filename/type/duration grammar now have bounded UTC-normalizing parsers, but their transport stays gated. Targeted DEX proves 6.36 ignores its IoT classification and always selects a manager that parses both device ID/password as Java signed `int`; production explicitly rejects camera 3's 64-bit ID and never fabricates an alias. The official catalog/API and signed `00.46.06.45` sideload APK are archived and verified; that build hides its real DEX behind `WrapperProxyApplication`/`libshella`, so obtain the Google Play 6.45.1 split set or recover the sideload DEX after runtime unpacking. Delete (`28/29`) and format stay disabled | wip |
