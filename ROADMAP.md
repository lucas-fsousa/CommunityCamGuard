...
| P1 | Shared PTZ D-pad independent of native/ONVIF transport: remove legacy arrow rendering based on `ptz_interaction`, use finite click/drag gestures on all PTZ-capable camera cards, and report Yoosee step support without requiring P2P enrollment. DOM and ONVIF dispatch contracts passed. Garagem/Quintal native transport remains separate: no durable P2P enrollments exist for them in the current DB; recover authentic enrollment credentials without guessing tokens or rebinding devices implicitly. See `docs/internal/ptz-model-selection.md` | wip |
