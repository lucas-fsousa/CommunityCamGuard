...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Baseline/delayed-ACCEPT replays preserved 151 video records / 155 audio frames. Fresh authenticated route, correlated meter replies, bounded AV reception/CLOSE and B9 release are composed. Same-process operator trigger now implemented: disabled by default, authenticated direct-loopback only, server-configured reviewed target, fixed three-second sample, single attempt per process and shared PTZ/audio/control lock. 274 selected tests passed, including architecture. Not deployed/activated; no camera transmission or production switch. Next: temporary gated deployment and one camera-3 live experiment, then keepalive timing/reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-operator-trigger.md`, `native-av-meter-diagnostic.md`, `native-av-route.md`, `native-av-close.md`, `native-av-probe.md`, `native-av-reorder.md`, `native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
