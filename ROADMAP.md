...
| P1 | Prefer homologated native driver transports per camera/feature, retaining RTSP/ONVIF fallback. PTZ: fixed type-2 codec, fresh camera-3 axis evidence, bounded gesture ownership and nonblocking prepared route implemented/tested offline; route preparation, live correlation, motion/STOP and browser lease integration pending. Native media then requires continuous reception, bounded local fan-out and measured latency/resource/recording comparison before replacing RTSP. No movement fallback after an ambiguous START. See `docs/internal/native-transport-preference.md` | wip |
