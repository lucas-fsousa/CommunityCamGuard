...
| P1 | Native video receive path: bounded KCP/MTP replay, cookie correlation and incremental V1 frame boundaries implemented. Historical 640×360 stream yielded 1,172 audio frames/1,137 video payloads, no boundary errors or timestamp regressions; 1080p prefix also parsed before the known KCP gap. Raw timestamps are not UTC. Isolated from working intercom/RTSP. Next: bounded independent codec decoding, keyframe/config transitions, pacing and single-producer handoff. See `docs/internal/native-media-receive.md` | wip |
