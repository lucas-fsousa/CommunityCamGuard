...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Baseline/delayed-ACCEPT replays preserved 151 video records / 155 audio frames. Fresh route owner composes authenticated preparation, one direct rendezvous, bounded AV reception/CLOSE and B9 release. Correlated meter responder and reviewed-target diagnostic now share the server's PTZ/audio/control lock; no unsolicited keepalives. 256 selected regression tests plus six architecture tests passed. No camera transmission or production switch; separate CLI processes do not share the lock. Next: gated same-process operator trigger and camera-3 live experiment, then keepalive timing/reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-meter-diagnostic.md`, `native-av-route.md`, `native-av-close.md`, `native-av-probe.md`, `native-av-reorder.md`, `native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
