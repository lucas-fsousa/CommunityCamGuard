...
| P1 | Complete browser microphone → AMR-NB mode 7/8 kHz → camera speaker two-way audio; legacy P2P transport and codec reproduce the native app. Production now has separate, tested MTP/KCP, StreamPipe, meter, AV, intercom-control, bounded OpenCORE encoder/audio sender and device-scoped orchestration layers. A live camera-3 run ACKed direct media, meter, AV, legacy START/header/talk ON/talk OFF/CLOSE and B9 teardown; local container validation produced ten exact AMR mode-7 frames, while offline transport tests prove 20 ms pacing, per-frame ACK backpressure and unconditional STOP/CLOSE after failure. Remaining: wire the guarded audio operation through the orchestrator, physical intelligibility confirmation, browser capture/backpressure and guarded driver/API/UI exposure | wip |
