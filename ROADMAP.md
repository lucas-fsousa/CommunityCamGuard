...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Reliable INIT/START and paired receive share a handshake owner with separate receipt/acceptance/encoding conditions and bounded pre-ACCEPT buffering. Baseline/delayed-ACCEPT replays preserved 151 video records / 155 audio frames. Socket adapter now implemented with 10-second lifetime, packet/byte budgets, cancellation and local cleanup; 19 fake-socket cases, 153 selected tests passed. No camera transmission or production switch; timestamps are not UTC. Next: reliable remote AV CLOSE, meter review, guarded fresh route and camera-3 live experiment, then reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-probe.md`, `native-av-reorder.md`, `native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
