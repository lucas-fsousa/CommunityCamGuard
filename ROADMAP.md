...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Reliable INIT/START and paired receive share a handshake owner with separate receipt/acceptance/encoding conditions and bounded pre-ACCEPT buffering. Baseline/delayed-ACCEPT replays preserved 151 video records / 155 audio frames. Socket probe now includes sequence-one AV CLOSE after reception: 10 seconds receiving plus 2 seconds for exact CLOSE receipt, shared traffic budgets and cancellation/local cleanup. 168 selected tests passed. Receipt is not route teardown proof. No camera transmission or production switch; timestamps are not UTC. Next: meter review, guarded fresh route with distinct B9 hangup cleanup and camera-3 live experiment, then reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-close.md`, `native-av-probe.md`, `native-av-reorder.md`, `native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
