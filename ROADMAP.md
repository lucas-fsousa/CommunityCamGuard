...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Baseline/delayed-ACCEPT replays preserved 151 video records / 155 audio frames. Fresh route/meter/AV CLOSE/B9 cleanup and gated same-process diagnostic are implemented. First camera-3 live invocation returned HTTP 502 after 4.26s; no media or teardown receipts confirmed, no retry. Diagnostic disabled/target cleared afterward; all three RTSP base streams showed one producer and advancing bytes. Added sanitized stage/cleanup logging with tests (not yet deployed) because original error did not identify failed phase. Next: deploy telemetry after green CI and collect one reviewed attempt before changing protocol/timeouts; native live video remains unhomologated. See `docs/internal/native-av-first-live-attempt.md`, `native-av-operator-trigger.md`, `native-av-meter-diagnostic.md`, `native-av-route.md`, `native-av-close.md`, `native-av-probe.md`, `native-av-reorder.md`, `native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
