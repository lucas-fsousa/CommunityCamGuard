...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR restart gate passed three 120-frame fresh-decoder recovery scenarios; ungated negative control failed (74/120). Paired-channel receive replay reached active with 151 video records/155 audio frames in ten seconds and zero pending bytes. Socket-free one-control INIT/START sender now has bounded identical retransmissions and exact ACK correlation (21 tests; historical control scan matched 12/14 receipts). Raw timestamps are not UTC; no packet transmission or production switch. Next: compose outbound and inbound negotiation ownership, bounded cross-channel reorder, camera-3 reception, reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-control-send.md`, `native-av-paired.md`, `native-video-recovery.md` and `native-media-receive.md` | wip |
