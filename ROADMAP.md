...
| P1 | Complete browser microphone → AMR-NB mode 7/8 kHz → camera speaker two-way audio; legacy P2P transport and codec reproduce the native app. Production now has separate, tested MTP/KCP, StreamPipe, meter, AV, intercom-control and device-scoped orchestration layers. A live camera-3 run through that production code ACKed direct media, meter, AV, legacy START/header/talk ON/talk OFF/CLOSE and B9 teardown in 11.6 s; the internal operation accepts no audio frames yet. Remaining: bounded AMR transport with physical intelligibility confirmation, browser capture/backpressure and guarded driver/API/UI exposure | wip |
