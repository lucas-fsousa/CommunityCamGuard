...
| P1 | Complete browser microphone → AMR-NB mode 7/8 kHz → camera speaker two-way audio; legacy P2P transport and codec reproduce the native app. Production now has separate, tested MTP/KCP, StreamPipe, meter, AV, intercom-control, bounded OpenCORE encoder/audio sender and device-scoped orchestration layers. A production-container camera-3 run encoded and sent 0.2 seconds of silence: direct media, meter, AV v1, header, talk ON, 10/10 mode-7 frame ACKs, talk OFF/CLOSE and B9 teardown all completed while RTSP/recording stayed active. Offline tests additionally prove 20 ms pacing, per-frame backpressure, an absolute session deadline and unconditional STOP/CLOSE after failure. Remaining: controlled physical intelligibility confirmation, browser capture/backpressure and guarded driver/API/UI exposure | wip |
