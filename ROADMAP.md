...
| P1 | Camera-card recordings — generic opaque-ID/UTC/bounded-page contracts and fail-closed Yoosee `tfInfo` parsing are implemented. Camera 3's real card repeatedly returns APK-defined normal status `1`, a stable card ID and coherent capacity/free values; falling free space proves active onboard recording. The parser handles the real `stVal`/integral-double envelope and retains firmware 40.1.14's signed-total quirk. The feature stays hidden until a listing probe proves this exact camera/profile. Exact bounded codecs and B9 BuiltIn carriers now cover modern V1 command `0` and V2-V4 command `16`, including V4 complete-fragment assembly; camera 3 ACKed V1/V2/V4 delivery/application but returned no list response, so retries of those variants are disabled and V3 will not be tried without selection evidence. The APK also contains an active `GLocalPlaybackVmImpl -> GPlaybackKits -> GwMsgGetSDAvList` UI path, whose 16-byte request and outer 20-byte remote-message envelope are recovered, but that manager parses IDs as signed Java `int` while camera 3 is explicitly classified as IoT and has a greater-than-32-bit ID. Transport selection/ID aliasing must be resolved statically or from an app playback trace before deciding whether any legacy session applies. Delete (`28/29`) and format stay disabled | wip |
