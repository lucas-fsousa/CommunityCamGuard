...
brand-agnostic; camera-specific capabilities remain owned by drivers.

| Priority | Item / acceptance criteria | Status |
| --- | --- | --- |
| P1 | **Recordings playback reliability/performance**: user reports long startup, loaded-but-paused video and needing to select the same row a second time before playback starts; downloading and playing locally is currently more reliable. Reproduce first-selection/reselection, seek and rapid switching. Measure API preparation, first byte, browser readiness and `play()` outcomes; inspect HTTP Range/206, media indexing/container layout, on-demand conversion and stale player/request races. Fix from evidence, without whole-recording browser/server buffering or duplicate/unbounded ffmpeg processes. Verify long recordings and mobile/desktop browsers; retain download behavior | todo |
| P1 | **Settings screen**: inventory `.env` options and classify runtime-editable, restart-required and server-only. Expose only an explicit safe allowlist with validation, persistence, localized explanations and authorization; never render the raw `.env` or secrets. Main server login key stays exclusively in `.env`, replaceable only by the server operator | todo |
| P1 | **Multiple temporary access keys**: retain main-key login and add independently generated dashboard keys with mandatory configurable expiration, labels, creation/expiry/revocation metadata and management restricted to sessions authenticated with the main key. Store only non-recoverable verifiers in the database; show a new key once, never log it. Temporary-key sessions cannot create/manage keys or elevate to main-key permissions | todo |
| P1 | **Immediate expiry/revocation enforcement**: associate each temporary session with its key and enforce expiry/revocation server-side on every protected operation, not only at login. Invalidating a key must invalidate all sessions derived from it, terminate authorized long-lived channels as applicable, and return open dashboards to login promptly via an authenticated invalidation mechanism. No reliance on browser clock, page refresh or a long-lived stateless cookie. Cover multiple tabs/devices, server restart and revoke-during-use races; main-key sessions remain independent | todo |
| P1 | **Authentication abuse resistance**: bounded/rate-limited login attempts for both key types, non-enumerating failures, cryptographically generated temporary keys, safe cookie/session handling and CSRF/origin protections. Review reverse-proxy trust so forwarded headers cannot bypass controls or spoof rate-limit identity; avoid unbounded limiter state or easy global lockout. Audit without brute-force traffic against production | todo |
