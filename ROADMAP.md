...
| P1 | Camera-card recordings — generic opaque-ID/UTC/bounded-page contracts and fail-closed Yoosee `tfInfo` parsing are implemented. Camera 3's real card repeatedly returns APK-defined normal status `1`, a stable card ID and coherent capacity/free values; falling free space proves active onboard recording. The feature stays hidden until a harmless listing probe proves this exact camera/profile. Bounded codecs cover modern V1 command `0`, V2-V4 file command `16`, V2-V4 date command `18`, and internal V3/V4 recording-type command `15`, including V4 fragment assembly. The vendor-signed Google Play 6.45 XAPK proves the UI route `PlaybackApiImpl → VPlaybackActivity → VSdcardPlaybackVM → PlaybackFileMgr`, selector, request bodies and transport-neutral eight-byte BuiltIn message. Native RE now pins its carrier chain as `MessageMgr → iv_send_passthrough_msg → giot_eif_send_passthrough_msg → iv_gutes_add_send_pkt`: it does use B9, but only after SDK-owned device-route selection, flags, sequencing, encryption and response bookkeeping. BA is an immediate GAT transport receipt, not proof of application execution. `iv_access_init` registers `_rcv_passthrough_cb`, which correlates the inner request ID at offset 4 and passes `payload[8:]` to the bounded list parser; outer message ID and inner request ID are independent. This transport-neutral response dispatcher is now implemented and tested without B9/socket dependencies. Native reverse-xrefs also correct the platform-version source: V2 is recorded only when received `PushStreamDistribute` byte `+0x18` has bit 0 set; MTP meter flags do not establish it. Decrypted GAT type `E4` is now a passively decoded, size/relay-bounded and device-correlated authoritative platform source, but no request path is exposed until that frame is observed through an equivalent session. Card-download payloads now have socket-free codecs: commands 19–23 cover correlated metadata, cancellation, `start_ms + resume offset`, error and bounded raw chunks without accumulating a file in RAM. Thumbnail commands 26/27 are likewise decoded with request-set correlation, 500-item batches, 30 KiB chunks and a 4 MiB/item ceiling, without assembling images or exposing a live transport. Playback lifecycle now includes strict command-4 stream-begin/command-17 EOF decoders plus proven empty pause/resume bodies and the command-3 `seek_ms + recording_start_ms` body; seek completion still requires both ACK and matching media. Playback speed command 24 now has fail-closed codecs for the app's 1×/2×/4×/8× choices and its distinct 4-byte platform-1/6-byte platform-2 layouts; unknown platform metadata or rates are rejected. Native `SDPlaybackPlayer` construction additionally proves that playback starts on a distinct connection route: its exact 32-byte A4 request-userdata carries selected/file-start milliseconds, source/camera IDs, initial rate and a platform-specific auxiliary value, while connection type 2 becomes definition bit `0x40` at A4 `+0xB0`. Native AV INIT repeats connection type `2` and the same exact 32-byte userdata at offsets `+0x10` and `+0x18`; the socket-free codec/session now express this route while preserving the live-view golden frame byte-for-byte. A strict socket-free codec and paired opt-in A4 fields are implemented; SD also advertises the native `PushStreamDistribute` option `0x4000`, while omitted fields preserve the homologated live-view bytes. No playback runtime path is exposed. Command-25 strategy values remain unmapped and disabled. Two manually assembled B9 experiments received transport ACK/BA but no data and caused visible LED activity. The unsafe manual B9 builder/parser/exchange has been deleted; all live playback entrypoints remain fail-closed before socket creation. Recover an SDK-equivalent standalone route and observe authoritative platform metadata, then confirm card enumeration in the current manufacturer app before any live query. The legacy `3→4` parser remains gated by its signed-32-bit-ID manager. Delete (`28/29`) and format remain disabled | wip |
