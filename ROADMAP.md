...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR restart gate passed three 120-frame fresh-decoder recovery scenarios; ungated negative control failed (74/120). Socket-free receive lifecycle now preserves acknowledged fragments/reordering through initialization handoff, with bounded progress and absolute deadlines; synthetic tests only, no production caller. Raw timestamps are not UTC. No live production switch. Next: correlated AV initializer plus continuous TLV/V1 ownership, camera-3 reception, reconnect/pacing/A-V sync and bounded single-producer handoff. See `docs/internal/native-receive-lifecycle.md`, `native-video-recovery.md` and `native-media-receive.md` | wip |
