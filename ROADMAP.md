...
| P1 | Native PTZ: user confirmed rightward 200ms movement and prompt STOP on camera 3. Integrate exact-unit/right-only opt-in with finite clicks, shared camera operation lock, automatic release and no fallback after attempted START. Other directions retain ONVIF; all-direction proof, session reuse and continuous browser leases remain pending | wip |
