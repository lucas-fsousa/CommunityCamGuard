...
| P1 | Complete browser microphone → AMR-NB mode 7/8 kHz → camera speaker two-way audio; legacy P2P transport and codec reproduce the native app. Production now has separate, tested MTP/KCP, StreamPipe, meter, AV, intercom-control, bounded OpenCORE encoder/audio sender and device-scoped orchestration layers. Camera 3 completed both a 10/10-frame silent run and a 30/30-frame low-level 740 Hz run; its own microphone returned the matching ~0.65-second band up to 27.6 dB above baseline, proving server-to-speaker playback acoustically while RTSP/recording stayed active. A generic authenticated/LAN-only API accepts at most ten seconds of exact PCM under the per-camera lock; the dashboard captures via AudioWorklet, resamples in memory, previews locally and sends only after confirmation. The transport sender accepts individually paced frames with per-frame ACK/backpressure, bounded lifetime and no catch-up bursts; the control lifecycle exposes explicit start/frame/close phases with idempotent cleanup. A separate PCM-stream orchestrator owns encoder, direct route, control state and teardown under the existing device renewal lock, behind a driver-neutral iterator contract. The authenticated/LAN-only WebSocket bridges exact 20 ms frames through a 500 ms queue and blocking worker with idle/session limits and cleanup. A separate hold-to-talk dashboard module performs streaming resampling and opens microphone/channel only while explicitly pressed. Deployed camera 3 completed a 200 ms WebSocket run with 10/10 ACK, full session/route cleanup and uninterrupted recorder PIDs/segments. Remaining: human-speech intelligibility through the actual browser microphone | wip |
