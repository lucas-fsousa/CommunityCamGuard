...
| P1 | Native video bootstrap follow-up: SDK LAN-A4 callback is non-gating; experimental AV requires a sent-meter roundtrip. Camera-3 sample after green CI `6bcd0a5`: HTTP 502/4.18s, two peer datagrams, broad meter=true but roundtrip=false; no AV INIT, B9 receipt=true, diagnostic disarmed and base RTSP ingress advancing. Added fixed-label meter classification for the next reviewed attempt (not yet deployed). Existing intercom unchanged. This supersedes the older A4 investigation next-step above. See `docs/internal/native-av-sdk-bootstrap.md` | wip |
