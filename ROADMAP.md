...
| P1 | Camera-card recordings — generic opaque-ID/UTC/bounded-page contracts and fail-closed Yoosee `tfInfo` parsing are implemented. Camera 3's real card repeatedly returns APK-defined normal status `1`, a stable card ID and coherent capacity/free values; falling free space proves active onboard recording. The feature stays hidden until a listing probe proves this exact camera/profile. Bounded codecs cover modern V1 command `0`, V2-V4 file command `16`, V2-V4 date command `18`, and internal V3/V4 recording-type command `15`, including V4 fragment assembly. The vendor-signed Google Play 6.45 XAPK proves the UI route `PlaybackApiImpl → VPlaybackActivity → VSdcardPlaybackVM → PlaybackFileMgr`, selector, request bodies and eight-byte carrier. Callback RE corrected response correlation to the outer B9 `message_id` and permits only the expected or cleared response command. Two corrected, bounded V2 daily reads on camera 3 then received ACKs and exactly one BA receipt but no B9 data; both caused visible LED activity. Raw BuiltIn playback over the bare brokered route is therefore unsafe/not SDK-equivalent on firmware 40.1.14. Live entrypoints for 0/15/16/18 now fail closed before socket creation; recover the SDK's prerequisite connection/routing state and confirm card enumeration in the current manufacturer app before any further live query. The legacy `3→4` parser remains gated by its signed-32-bit-ID manager. Delete (`28/29`) and format remain disabled | wip |
