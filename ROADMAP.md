...
| P1 | Map and expose the camera's **selectable siren sound/effect** — query/write contracts and silent selection/restore are proven; production now has a sanitized type-4 AMR catalogue codec, non-secret semantic option keys, fixed read-only C0/C1 framing, bounded fragment/session orchestration with an explicit fail-closed decoder seam, and an unregistered selection codec that accepts only freshly decoded resources with preflight/readback. Remaining: legally compatible portable QuickLZ level-2/stream-1024 decode, catalogue integration/API/UI and camera-3 dashboard validation | wip |
