...
| P1 | Camera-card recordings — generic opaque-ID/UTC/bounded-page contracts and fail-closed Yoosee `tfInfo` parsing are implemented. Advertise only after positive capacity/status plus a verified model/profile or successful read-only probe; all current units correctly remain hidden because they report no card. Modern IoTVideo commands `0/16/18/19..27` are mapped, and the exact 43-byte V2 command-16 list body now has a tested codec. The app's active legacy `GwMsgGetSDAvList` path and exact 16-byte local-wall-clock request are also recovered and covered by fixtures. Response parsing/download framing still need a real card capture; V3/V4 options, delete (`28/29`) and format remain disabled | wip |
