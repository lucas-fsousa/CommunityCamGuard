...
| P1 | Map and expose the camera's **selectable siren sound/effect** — query/write contracts and silent selection/restore are proven; production now has a sanitized type-4 AMR catalogue codec, non-secret semantic option keys, fixed read-only C0/C1 framing, bounded fragment reassembly/ACK and an unregistered selection codec that accepts only freshly decoded resources with preflight/readback. Remaining: legally compatible portable QuickLZ decode/session orchestration, explicit catalogue API/UI and camera-3 dashboard validation | wip |
