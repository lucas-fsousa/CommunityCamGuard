...
| P1 | Native video bootstrap follow-up: SDK LAN-A4 callback is non-gating; transfer selection uses MTP meter quality. Experimental AV now requires an exact sent-meter roundtrip instead of LAN A4 receipt plus broad meter observation. Existing intercom unchanged. Next: green CI, then separately armed camera-3 sample; no live confirmation yet. This supersedes the older A4 investigation next-step above. See `docs/internal/native-av-sdk-bootstrap.md` | wip |
