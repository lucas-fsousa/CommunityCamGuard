...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Baseline/delayed-ACCEPT replays preserved 151 video records / 155 audio frames. Fresh route owner now composes authenticated preparation, one direct rendezvous, metering, bounded AV reception/CLOSE and separate B9 release before socket closure. Route ID survives ambiguous failures; strict release receipt correlation and per-phase traffic/time budgets added. 201 selected tests passed. No camera transmission or production switch; receipts are not physical teardown proof. Next: meter/keepalive review, internal camera-3-only invocation under operation reservation and live experiment, then reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-route.md`, `native-av-close.md`, `native-av-probe.md`, `native-av-reorder.md`, `native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
