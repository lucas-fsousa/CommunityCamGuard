...
| P1 | Complete browser microphone → AMR-NB mode 7/8 kHz → camera speaker two-way audio; legacy P2P transport and codec reproduce the native app. The media harness now consumes the app's exact encrypted enrollment, guarantees native B9 route teardown and completed a camera-3 silent control run: direct media, AV ACCEPT/START, legacy microphone START/STOP/CLOSE and teardown were all ACKed with zero audio frames sent. Remaining: physical AMR intelligibility confirmation, browser capture/backpressure and production integration | wip |
