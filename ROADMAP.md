...
| P1 | Map and expose the camera's **selectable siren sound/effect** — production now includes the sanitized type-4 catalogue, bounded C0/C1 fragment/session flow, independent memory-safe QuickLZ level-2 decoder, generic dynamic-option API/UI and guarded selection with fresh-catalogue resolution, preflight and readback. Camera 3 returned all four Portuguese effects through the production driver and its already-active `Zumbido 1` key passed idempotently without a write or playback. Remaining: interactive validation in the rebuilt dashboard and one daytime reversible change/restore | wip |
