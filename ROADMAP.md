...
| P1 | Map and expose the camera's **selectable siren sound/effect** — query/write contracts and silent selection/restore are proven; production now has a sanitized type-4 AMR catalogue codec, non-secret semantic option keys, fixed read-only C0/C1 framing, bounded fragment/session orchestration, an independent memory-safe QuickLZ level-2 decoder, and an unregistered selection codec that accepts only freshly decoded resources with preflight/readback. The private system+custom catalogue orchestrator is live-proven read-only on camera 3 (four Portuguese effects, no custom entries). Remaining: dynamic-option application/API/UI contract and dashboard selection validation | wip |
...
| P1 | Camera-speaker volume — APK 0/25/50/75/100% positions, raw 0..10 read normalization and D3/readback are implemented in the production driver/API/UI; camera-3 dashboard validation remains | wip |
...
| P1 | Smart Protection — typed master on/off and weekly schedule are implemented in the production driver/API/UI with preflight/readback and without overwriting adjacent guard settings. Camera-3 dashboard-path validation remains | wip |
