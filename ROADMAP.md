...
| P1 | Map and expose the camera's **selectable siren sound/effect** — query/write contracts and silent selection/restore are proven; production now has a sanitized type-4 AMR catalogue codec and non-secret semantic option keys. Remaining: portable fragmented/QuickLZ service transport, explicit catalogue API/UI and camera-3 dashboard validation | wip |
