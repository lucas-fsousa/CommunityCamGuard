...
| P1 | Complete browser microphone → AMR-NB mode 7/8 kHz → camera speaker two-way audio; legacy P2P transport and codec reproduce the native app. Native brokered route teardown (`B9` inner type 0 / reason `0x4e22`) is now recovered and production-proven across four consecutive camera-3 direct routes; wire it into the media harness before the next physical intelligibility test | wip |
