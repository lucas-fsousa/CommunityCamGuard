...
| P1 | Native video receive path: bounded KCP/MTP/V1 replay and independent HEVC/AAC decoding validated offline. Configured-IDR recovery passed three 120-frame scenarios; ungated control failed (74/120). Reliable INIT/START sender and paired receiver share a socket-free handshake owner with separate receipt/acceptance/encoding conditions. Cross-channel START/media before ACCEPT now waits encrypted within 128 messages / 256 KiB / 1 second, with terminal cleanup on failure. Baseline and delayed-ACCEPT ten-second replays both preserved 151 video records / 155 audio frames; 134 selected tests passed. Raw timestamps are not UTC; no packet transmission or production switch. Next: camera-3 socket/reception experiment, reconnect/pacing/A-V sync and single-producer handoff. See `docs/internal/native-av-reorder.md`, `native-av-handshake.md`, `native-av-control-send.md`, `native-av-paired.md` and `native-video-recovery.md` | wip |
