...
def test_delete_camera_removes_it():
    camera = _seed_camera()
    out = camera_routes.delete_camera(camera.camera_id, _req(media=None, rec=None))
    assert out == {"ok": True}
...
def test_probe_camera_success(monkeypatch):
    ...
    monkeypatch.setattr(
        camera_routes,
        "probe_and_store",
        lambda cam: registry.get_camera(cam.mac),
    )
    out = camera_routes.probe_camera(camera.camera_id)
    assert out["mac"] == MAC
...
def test_reboot_success(monkeypatch):
    camera = _seed_camera()
    monkeypatch.setattr(camera_routes.drivers, "for_camera", lambda cam: FakeDriver(reboot_result=True))
    assert camera_routes.reboot_camera(camera.camera_id) == {"ok": True, "rebooting": True}
...
def test_legacy_mac_reference_remains_temporarily_accepted(monkeypatch):
    ...
    _seed_camera()
    monkeypatch.setattr(camera_routes.drivers, "for_camera", lambda cam: FakeDriver(reboot_result=True))
    assert camera_routes.reboot_camera(MAC) == {"ok": True, "rebooting": True}
...
def test_discovery_scan_returns_configured_and_candidates(monkeypatch):
    registry.init_db()
    monkeypatch.setattr(discovery_routes.active_scan, "scan", lambda **k: [])
    monkeypatch.setattr(
